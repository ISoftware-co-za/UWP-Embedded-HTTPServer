...
{
    internal class LogsDeleteProcessor : ILogsRequestProcessor
    {
        public async Task<HTTPResponse> GenerateResponse(HTTPRequest request, StorageFolder logsFolder)
        {
            IReadOnlyList<StorageFile> logFiles = await logsFolder.GetFilesAsync();

            foreach (var logFile in logFiles)
                await logFile.DeleteAsync();
            return CommonResponses.FoundRedirect("/logs");
        }
    }
}
