...
namespace ISoftware.UWP.HTTPServer
{
    ...
    /// </summary>
    /// <remarks>
    /// The HTTP server can be embedded in Windows universal applications. It can, amongst other things, be used to control the application, get the application status, and view logging files.
    /// </remarks>
    public class Server
    {
        #region PROPERTIES
        ...
        /// <summary>
        /// Gets the boolean value indicating if the <see cref="Server"/> is running and can process HTTP requests.
        /// </summary>
        /// <value>Running returns <c>true</c> if the <see cref="Server"/> can accept and process requests; otherwise the <see cref="Server"/> is not running and <c>false</c> is returned.</value>
        public bool Running { get; private set; }

        /// <summary>
        /// Gets the router for the <see cref="Server"/>. The router links HTTP path prefixes to <see cref="IRequestProcessor"/> instances. These instances are used to process HTTP requests whose path starts with the specified prefix.
        /// </summary>
        /// <value>The router instance.</value>
        public PathRequestProcessorRouter Router { get; }

        /// <summary>
        /// The list of <see cref="HTTPHeader">HTTPHeaders</see> that will be added to all responses generated by the <see cref="Server"/>.
        /// </summary>
        /// <value>The list of default headers.</value>
        public List<HTTPHeader> DefaultHeaders { get; }

        /// <summary>
        /// Gets or sets the boolean value that determines if the <see cref="Server"/> will log the requests and associated responses.
        /// </summary>
        /// <value>The boolean value that is <c>true</c> if the <see cref="Server"/> logs requests and responses; otherwise the <see cref="Server"/> does not perform logging and <see cref="VerboseLogging"/> will be <c>false</c>.</value>
        ...
        /// </summary>
        /// <param name="port">The port on which th server listens for new requests.</param>
        /// <param name="readTimeoutMSecs">The request read timeout in milliseconds.</param>
        /// <param name="logger">The logger. If loffing is not required then can be <c>null</c>.</param>
        public Server(int port, int readTimeoutMSecs, ILogger logger)
        {
            ...
            DefaultHeaders = new List<HTTPHeader>();
        }
        ...
        /// <summary>
        /// Occurs when the <see cref="Server"/> completes the processing of a request.
        /// </summary>
        ...
        /// <summary>
        /// Handles the <see cref="E:ConnectionReceived" /> event.
        /// </summary>
        /// <param name="sender">The sender.</param>
        /// <param name="args">The <see cref="Windows.Networking.Sockets.StreamSocketListenerConnectionReceivedEventArgs" /> instance containing the event data.</param>
        private async void OnConnectionReceived(StreamSocketListener sender, StreamSocketListenerConnectionReceivedEventArgs args)
        {
            ...
            try
            {
                ...
                if (VerboseLogging && _logger != null)
                {
        ...
        private async Task WriteResponse(StreamSocketListenerConnectionReceivedEventArgs args, HTTPResponse httpResponse)
        {
            using (IOutputStream output = args.Socket.OutputStream)
            {
                ...
                if (DefaultHeaders.Count > 0)
                    responseHeader = DefaultHeaders.Aggregate(responseHeader, (current, header) => current + $"{header.Name}:{header.Value}\r\n");
        ...
        /// </summary>
        /// <returns>Windows.Networking.HostName.</returns>
        ...
        /// <returns>ISoftware.UWP.HTTPServer.HTTPRequest.</returns>
        ...
        /// </summary>
        /// <param name="queryParameterString">The query parameter string.</param>
        /// <returns>System.Collections.Generic.Dictionary&lt;System.String, System.String&gt;.</returns>
